...
function TrackTicketSection() {
    ...
    const handleSearch = async (e) => {
        ...
        setError(null);
        try {
            const data = await ticketService.checkTicketStatus(ticketId);
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">{searchResult.ticket_number}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        {searchResult.assigned_user && (
                            <div className="flex justify-between">
                                <span className="text-gray-600">Ditangani oleh:</span>
                                <span className="font-medium">{searchResult.assigned_user.name}</span>
                            </div>
                        )}
                    </div>
                </div>
...
export default function StaffSupportDashboard() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (activeTab === 'assigned') {
                params.status = 1; // Assigned/Pending
            } else if (activeTab === 'completed') {
                params.status = 4; // Completed
            }
            // For 'technical' and 'support' tabs, show all tickets

            const response = await ticketService.getTickets(params);
            let filteredTickets = response.data || [];

            // Additional filtering for technical and support tabs
            if (activeTab === 'technical') {
                filteredTickets = filteredTickets.filter(t => 
                    t.category?.name?.toLowerCase().includes('it') || 
                    t.category?.name?.toLowerCase().includes('hardware') ||
                    t.category?.name?.toLowerCase().includes('software') ||
                    t.category?.name?.toLowerCase().includes('network')
                );
            } else if (activeTab === 'support') {
                filteredTickets = filteredTickets.filter(t => 
                    t.category?.name?.toLowerCase().includes('general')
                );
            }
            ...
            setStats({
                assigned: ticketData.filter(t => t.status_id === 1).length,
                inProgress: ticketData.filter(t => t.status_id === 2).length,
                resolved: ticketData.filter(t => t.status_id === 4).length,
                technical: ticketData.filter(t => {
                    const catName = t.category?.name?.toLowerCase() || '';
                    return catName.includes('it') || catName.includes('hardware') || 
                           catName.includes('software') || catName.includes('network');
                }).length
            });
        } catch (err) {
    ...
    const getTypeIcon = (ticket) => {
        const catName = ticket.category?.name?.toLowerCase() || '';
        if (catName.includes('it') || catName.includes('hardware') || 
            catName.includes('software') || catName.includes('network')) {
            return "🔧";
        }
        return "💬";
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-600">
                                                    {ticket.ticket_number} • {ticket.requester?.name || 'N/A'}
                                                </p>
                                            </div>
                                        </div>
                                        
                                        <div className="flex items-center gap-3">
                                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${getPriorityColor(ticket.priority?.name)}`}>
